...
| Artefact | Role |
|---|---|
| Repository | Code, specs, and history. The history is evidence of how the decisions were reached |
| `README.md` | Entry point: what it is, how to run it, how to call it, measured results |
| **`BENCHMARKS.md`** | **The centrepiece.** Measured performance and cost, with methodology stated ([09](09-benchmarks.md)) |
...
`BENCHMARKS.md` carries the most weight. A working endpoint demonstrates the task was completed; a rigorous benchmark demonstrates the deployment was *understood*: where the time goes, what it costs, where it breaks, and which GPU is right rather than assumed.
...
| Weights | **Cached models.** RunPod pre-stages the repo on hosts. The baked image stays a build target, since the brief asks for an image containing the model; it is not published |
| Build host | Local, or a GitHub runner via `deploy.yml`. No Pod: the deployed image carries no weights and is 2.92GB |
| Registry | GHCR |
...
The deployed endpoint nevertheless uses **cached models**: RunPod pre-stages the HuggingFace repository on host machines before a worker starts, preferring hosts that already hold it. Both images build from one Dockerfile via `BAKE_WEIGHTS`. The baked one is a one-command build target (`make build-baked`), **buildable on demand and not published**: the artefact the brief names is reproducible from this repo, but pushing ~45GB to GHCR buys nothing the build target does not already prove. The cached endpoint serves traffic.
...
Staging pulls the *whole* repository, so the ~24GB of duplicate single-file weights come along: unbilled, not our disk, and the reason scale-up is measured rather than assumed.
...
Cached models mount at `/runpod-volume/huggingface-cache/hub`, the same path a network volume uses. Attaching both would put two mechanisms on one mount point.
...
**1. `core/` depends on nothing outward.** It declares `Protocol` interfaces; `adapters/` implements them. Enforced by `import-linter`, not review. This makes each additional transport a small addition rather than a rewrite, and lets the entire gateway be built and tested before a RunPod endpoint exists.

**2. No unit or integration test may require a GPU, weights, or an external network service.** A design constraint, not a testing preference: it forces the pipeline behind a lazily-initialised injectable accessor, the difference between a testable handler and an untestable one.
...
So `POST /v1/jobs` → `job_id`, `GET /v1/jobs/{id}` → result is the only interface. No synchronous endpoint: it would bound concurrency by held connections rather than GPU capacity, and on a cold worker (30-60s of pipeline load before 20-25s of generation) it would exceed any deadline safe against load-balancer idle timeouts. The convenience path would be the one most likely to look broken on a first call. `client/generate.py` covers the two-call ergonomics instead.

Async is the gateway's interface. It does not change what the **worker** returns: RunPod serverless has a fixed surface, `GET /status/{job_id}` returns the handler's output verbatim, so the worker returns base64 by default ([01](01-worker.md)). Storage references are opt-in: a key is only resolvable by a caller running the gateway, and the graded tier must not depend on the ungraded one.

## Sequencing

**All phases below are complete as of 2026-08-06.** The plan is kept as written, because the *shape* of it is the point: 2b was the graded deliverable and the only phase blocked on RunPod credits, so everything else was sequenced to run locally and reduce 2b to pure execution. It collapsed into one sitting when the credits landed.
...
| 3 | Gateway core, adapters (in-memory, not Postgres), auth, tests | Coverage gate | No |
...
Splitting 2a from 2b makes the wait productive: it verifies both build-blockers in [06](06-build-deploy.md) with no GPU and no download.
...
That rule held: no figure was stated as measured before 2b ran. `BENCHMARKS.md` now carries 156 records with `benchmarks/raw.jsonl` committed beside it.
...
| ~45GB image push is slow and failure-prone | Resolved by not shipping weights: the deployed image is 2.92GB, built locally or in `deploy.yml`, no Pod. GHCR avoids Docker Hub pull limits |
...
1. **Weights are not in the deployed image.** Superseded twice: first by a network volume, then by cached models, which removed the volume's datacenter pin and storage bill. The baked image remains a build target because the brief names it, but it is not published (see *Why cached models* above).
