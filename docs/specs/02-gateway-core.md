...
`ErrorCode` is an enum, not a string. The error-code contract requires 100% test coverage ([`STANDARDS.md`](../../STANDARDS.md) §9), not achievable if any string can be written into the field.
...
`Clock` is injected rather than calling `datetime.now()`: timestamp assertions in tests need a fixed clock, and a service that reaches for wall time cannot be tested deterministically.
...
**A shed request releases its key.** A `429` row is written FAILED, and a key left bound to it would replay that failure for the whole retention window — the retry the `Retry-After` asks for could never become a real attempt. Nothing was submitted upstream, so there is no duplicate GPU work to protect against. Every other terminal state, including a genuine failure, keeps its binding.
...
`CANCELLED` has two producers. `POST /v1/jobs/{id}/cancel` ([03](03-facades.md)) sets it, calling RunPod's `POST /v2/{endpoint_id}/cancel/{job_id}` so the billing stops upstream too; and RunPod can report it independently, for instance when a worker is reclaimed, which is why an unmapped upstream status must not become an unhandled case either way.

`/retry` and `/purge-queue` exist on the same upstream API and would be the same shape of addition: a route plus one adapter call. Neither is built; neither requires redesign.
...
Comparison is constant-time via `hmac.compare_digest`. The stored value is a fixed-length digest, so the length-leak argument does not apply. The reason is the prefix leak: `==` short-circuits on the first differing byte, so comparison time correlates with how much of a guess is correct, enough to walk a digest byte by byte.
...
Rotation means editing settings and restarting. Acceptable at this scale; recorded as a gap in [08](08-production-readiness.md).
...
Breaker state is per-process and in-memory. Correct for a single instance, wrong for a fleet; noted in [08](08-production-readiness.md).
...
**What this adds over RunPod's own queue is narrow.** RunPod already queues, tracks status, retries, and serves results. It does exactly two things we need and it does not do: it **deletes results 30 minutes after completion**, and it has no notion of *which caller* a job belonged to. Durability past that window, and attribution, are the whole justification for a job store. Everything else here is the platform's work, not ours.
...
The rule: a claimed job with no `runpod_job_id` is **resubmitted**, not polled. Safe because no upstream job exists to duplicate: that is precisely what the null means. Jobs older than the deadline are timed out as usual, so a permanently failing resubmit cannot loop indefinitely.
...
600s sits inside RunPod's **30-minute result retention** for `/run` jobs (a job resolved later than that would find nothing left to fetch) and well above the 300s execution timeout ([01](01-worker.md)) plus plausible queue wait. **The gateway deadline must always exceed the endpoint execution timeout.** Set below it, we would time out jobs still running normally, then discard their results when they complete.

### Where it runs

**Here:** an asyncio task started in the FastAPI `lifespan` hook, in the same process. One container, nothing extra to run, which keeps `docker compose up` to a single command ([06](06-build-deploy.md)).
...
estimated_wait  = (jobs.inQueue / capacity) * AVG_JOB_S
```
...
Two caveats. `capacity` counts a *running* worker as available, which it is not until its current job finishes, so the estimate is a lower bound and real waits skew longer. The jitter is not decoration: without it every shed client returns at the same instant, converting one queue spike into a synchronised second one.

Rejecting on estimated time rather than raw depth means the threshold survives a change to `max_workers`, and it makes `Retry-After` a real number rather than a guess. `AVG_JOB_S` starts at a stated estimate and is replaced by the measured p50 from [09](09-benchmarks.md).
...
Five files are committed at the repository root: `contracts/generation-request.schema.json`, `contracts/error-codes.json`, `contracts/blocklist.json`, `contracts/normalisation.json` and `contracts/guardrail-corpus.json`. Each package has a test asserting its own definitions match the committed contract: the worker that it accepts every field, the gateway that it sends them, and both that their error-code sets are identical.
...
This is the honest cost of the isolation rule, paid explicitly. Silent drift between two copies of a contract is a nasty failure: the gateway accepts a request the worker rejects, and it only shows up in production.
