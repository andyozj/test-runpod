...
The other motive is curiosity. The endpoint was live and the obvious questions (what a step costs, when cold start hurts, which GPU wins on $/image) deserved measured answers over guesses. Small-N characterization answers them; it is not load testing, and the report says so.
...
**Named output: `AVG_JOB_S`.** The p50 at the default configuration (1024², 28 steps, on the recommended GPU) is the constant driving the queue-pressure threshold in [02](02-gateway-core.md#queue-pressure). Currently an estimate ([08](08-production-readiness.md) gap #9); this sweep replaces it. Recorded explicitly as config output, not left to be inferred from a table.
...
Storage adds nothing for the deployed variant: cached models are staged by the platform and bill nothing, and no images are stored. The baked variant's only storage cost is the registry. An earlier design carried a per-GB monthly bill that grew whether or not anyone submitted a request.
...
One row earns its place: it is the only latency anyone experiences, and the reconciler tick is a design parameter. A delta dominated by the tick is an argument for a shorter tick or for SSE, decided from data rather than taste.
...
The third endpoint costs almost nothing to add: it reuses the ~10GB volume image with no volume attached. The **Model** field on the endpoint names the HF repo, RunPod pre-stages it on host machines before the worker starts (unbilled), and `WEIGHTS_PATH` is left **unset** so the worker falls through to `MODEL_CACHE_ROOT` and discovers the staged snapshot itself ([01](01-worker.md#weight-path), `docs/RUNBOOK.md`). Setting `WEIGHTS_PATH` there would win over the cache and leave the pre-staged weights unused. Cached models are RunPod's stated recommendation for HF-hosted models, so a weight-delivery comparison that omits them measures yesterday's platform. If 2b time runs short, this endpoint is the one dropped; the two-way comparison stands on its own.

**Precondition, run first.** Every endpoint must report the same `sha256` over its weight files ([07](07-testing.md)). If they differ, one is running a different revision (a stale volume, a stale image, or a cache staged at a different time), and every row below compares two variables at once. The worker reports the revision it loaded on every result (`model_version`), which is what makes the mismatch visible; there is no pinned SHA to compare against. **The comparison is void until this passes.**
...
Not a performance measurement, but what makes the performance measurement *actionable*: if 20 steps is visually indistinguishable from 28, the 28-step latency is the wrong default and the benchmark has changed a decision rather than just described one.
...
- A `--only` subset refuses to render `BENCHMARKS.md`. Rendering reads the whole of `raw.jsonl`, so a partial run would publish a report whose sections were measured against different images.
...
Values that feed configuration (`AVG_JOB_S`, the recommended GPU, the recommended variant) are listed together at the end of the summary, so the report has an actionable output and not only a descriptive one.
...
- **Small N.** N=10 per sweep cell, N=3 for cold start and payload. That bounds variance loosely; confidence intervals are not claimed, and a percentile over 3 samples is an ordering, not a distribution. No number in the report is literal. SLO-grade p50/p95 needs a sustained load run (e.g. Locust) at production concurrency, which this harness does not do.
