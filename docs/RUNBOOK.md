...
| New release deployed, behaviour unchanged | Rolling releases replace idle/busy workers (documented), but **FlashBoot-retained workers and API-driven template PATCHes are the uncovered corner** — a retained worker resumed on the old image post-update (2026-08-06, cost an hour) | The bounce, which the rolling-releases doc itself endorses for strict consistency: `workersMax` 0, ~20s, restore. `apply_endpoint.py` does it automatically |
