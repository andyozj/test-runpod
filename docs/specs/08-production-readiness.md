...
> **Status: reconciled against the shipped code on 2026-08-06.** Every row in *Built* below is implemented; every row in *Not built* is genuinely absent.
>
> This is the document most easily made false: every row in *Built* is a commitment, and a reviewer will check the ones that sound impressive. Reconciling it against the shipped code is a required step before submission, not a courtesy. Anything that does not ship moves to *Not built* with the reason. A spec that overstates what was delivered destroys the credibility of the parts that are accurate.
...
| Reproducibility | Seed always echoed, loaded revision discovered from the staged snapshot and reported as `model_version` on every result |
...
| Deploy automation | `deploy.yml`: CI gates (on the tag-push entry point too) → build/push → GHCR manifest verification of the tag → apply → post-deploy e2e smoke test. Rollback is the same workflow with the previous tag. The smoke test is detection, not prevention: apply has already run, so rollback is manual |
...
|---|---|---|---|
| 1 | **No per-caller rate limit or quota** | One caller can exhaust the budget. `MAX_ACTIVE_JOBS_PER_KEY` (default 10) bounds a key's *concurrent* non-terminal jobs, which is a share-of-queue control, not a request-rate limit and not a spend cap: a key that stays under the cap can submit indefinitely | Low: a counter table and middleware |
...
| 9 | **`AVG_JOB_S` is a constant** | The queue-wait estimate driving the `429` does not adapt to resolution, step count, or GPU. A 50-step 1536² job is estimated the same as a 20-step 512² one | Low: rolling p50 from completed jobs |
...
| 14 | ~~**No cancellation**~~ **Closed.** | `POST /v1/jobs/{id}/cancel` stops the job and calls RunPod's `POST /cancel` so the billing stops upstream too. Caller-scoped; a job already terminal is returned unchanged | Done |
| 15 | **Baked image build is outside CI** | ~45GB against ~14GB of runner disk. The deployed *slim* image (2.92GB) builds and pushes in `deploy.yml` on a standard runner; only the baked variant stays a manual `make build-baked` | Low: it is deliberate, not blocked |
...
**#4 cannot be fixed from here.** The endpoint is not open to the world; it requires the RunPod API key. The problem is the *shape* of that key: account-scoped, identical for everyone who holds it, capable of creating and deleting resources, and impossible to narrow to "may submit jobs to this one endpoint". There is no way to let something call the endpoint directly without also handing it the account.
...
FLUX.1-dev is released under a non-commercial licence. Fine for evaluation; a commercial deployment needs FLUX.1-schnell (Apache-2.0) or a commercial licence from Black Forest Labs. Not a constraint on this exercise.
...
4. **Load test**, to replace the reasoned concurrency and queue thresholds with measured ones, and to make `AVG_JOB_S` adaptive.
